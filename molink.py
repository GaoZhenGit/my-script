...
import os
import time
from tools import log_info, log_warn, log_error, C, G, Y, RED, R
...
def _rollback():
    ...
    if os.path.exists(OLD_EXE_PATH):
        try:
            ...
            log_error(f"回滚失败: {e}")
            log_error(f"请手动恢复: {OLD_EXE_PATH} -> {EXE_PATH}")


def update():
    log_info("开始更新 molink...")

    if not os.path.exists(EXE_PATH):
        log_error(f"molink.exe 不存在: {EXE_PATH}")
        return 1
    ...
    if os.path.exists(OLD_EXE_PATH):
        ...
        try:
            ...
            log_error(f"无法删除旧备份: {e}")
            return 1

    # Step 1: 确认 daemon 运行中
    log_info("检查 daemon 状态...")
    if not _is_running():
        log_info("daemon 未运行，正在启动...")
        try:
            _run_exe(EXE_PATH, ["start"], check=True, capture=False)
            time.sleep(1)
            if not _is_running():
                log_error("启动 daemon 失败")
                _status(EXE_PATH)
                return 1
            log_info("daemon 已启动")
        except subprocess.CalledProcessError as e:
            log_error(f"启动 daemon 失败: {e}")
            return 1
        except FileNotFoundError:
            log_error("找不到 molink.exe")
            return 1
    else:
        log_info(f"daemon 状态: {G}running{R}")

    # 检查是否有活跃的端口转发
    was_forwarding = _has_forwards()
    if was_forwarding:
        log_info(f"检测到活跃转发，更新后将恢复")

    # 获取旧版本号
    old_ver = _get_version(EXE_PATH) or "unknown"
    log_info(f"当前版本: {C}{old_ver}{R}")

    # Step 2: 改名（Windows 允许重命名运行中的 exe）
    log_info("备份旧版本 -> molink_old.exe")
    try:
    ...
    # Step 3: 拉取新版本
    log_info(f"从设备拉取新版本: {C}{REMOTE_EXE}{R}")
    try:
        result = _run_exe(OLD_EXE_PATH, ["pull", REMOTE_EXE, EXE_PATH])
        if result.stdout:
            print(result.stdout.strip())
        if result.returncode != 0:
            log_error(f"pull 失败: {result.stderr.strip() if result.stderr else '未知错误'}")
            _rollback()
            return 1
    except subprocess.CalledProcessError as e:
        log_error(f"pull 失败: {e}")
        _rollback()
        return 1
    except FileNotFoundError:
        log_error("找不到 molink_old.exe，可能已被误删")
        log_error(f"请手动恢复: {OLD_EXE_PATH} -> {EXE_PATH}")
        return 1

    # Step 4: 验证新文件
    if not os.path.exists(EXE_PATH) or os.path.getsize(EXE_PATH) == 0:
        log_error("拉取的文件无效（不存在或大小为0）")
        _rollback()
        return 1
    log_info(f"新文件大小: {C}{os.path.getsize(EXE_PATH):,} bytes{R}")
    ...
    try:
        ...
        if result.returncode != 0:
            log_warn(f"stop 返回非零: {result.stderr.strip() if result.stderr else ''}")
    except Exception as e:
        log_warn(f"stop 命令异常: {e}")

    # Step 6: 等待退出
    log_info("等待 daemon 退出...")
    time.sleep(2)
    stopped = False
    for i in range(5):
        if not _is_running(OLD_EXE_PATH):
            log_info(f"daemon 状态: {RED}stopped{R}")
            stopped = True
            break
        log_info(f"等待中... ({i + 1}/5)")
        time.sleep(1)
    if not stopped:
        log_warn("daemon 可能未完全停止，继续...")

    # Step 7: 验证新版本
    log_info("检查新版本...")
    new_ver = _get_version(EXE_PATH) or "unknown"
    log_info(f"版本变更: {C}{old_ver}{R} -> {C}{new_ver}{R}")
    ...
    try:
        ...
        log_warn(f"清理旧文件失败: {e}")
        log_warn(f"请手动删除: {OLD_EXE_PATH}")
    ...
    try:
        ...
        if _is_running():
            ...
            log_warn("daemon 可能未成功启动")
            _status(EXE_PATH)
    except subprocess.CalledProcessError as e:
        log_error(f"启动新 daemon 失败: {e}")
        return 1
    ...
    if was_forwarding:
        ...
        try:
            ...
            if result.returncode != 0:
                log_warn(f"恢复转发可能失败: {result.stderr.strip() if result.stderr else ''}")
            else:
                log_info("端口转发已恢复")
        except Exception as e:
            log_warn(f"恢复转发异常: {e}")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "update":
        try:
            ...
            print()
            log_warn("用户中断，请检查状态后再试")
            sys.exit(130)
        except Exception as e:
            log_error(f"更新过程异常: {e}")
            sys.exit(1)
    else:
